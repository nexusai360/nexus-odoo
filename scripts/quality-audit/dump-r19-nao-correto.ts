...
const MARKER = "[AUDIT-POS-2026-05-27T21-50-50]";
...
async function main() {
  ...
  console.log(`R19 nao-CORRETO: ${evals.length}`);

  const md: string[] = ["# Auditoria R19 (nao-CORRETO)", "", `Marker: ${MARKER}`, `Total: ${evals.length}`, ""];
  let n = 0;
  ...
  writeFileSync("/tmp/r19-nao-correto.md", md.join("\n"));
  console.log("Dump em: /tmp/r19-nao-correto.md");
