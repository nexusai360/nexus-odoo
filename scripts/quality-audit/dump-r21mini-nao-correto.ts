...
const MARKER = "[AUDIT-POS-2026-05-28T02-43-02]";
...
async function main() {
  ...
  console.log(`R21mini nao-CORRETO: ${evals.length}`);

  const md: string[] = ["# Auditoria R21mini (nao-CORRETO)", "", `Marker: ${MARKER}`, `Total: ${evals.length}`, ""];
  let n = 0;
  ...
  writeFileSync("/tmp/r21mini-nao-correto.md", md.join("\n"));
  console.log("Dump em: /tmp/r21mini-nao-correto.md");
