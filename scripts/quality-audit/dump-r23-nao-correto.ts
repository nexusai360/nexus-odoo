...
async function main() {
  ...
  console.log(`R23 nao-CORRETO: ${evals.length}`);

  const md: string[] = ["# Auditoria R23 (nao-CORRETO)", "", `Marker: ${MARKER}`, `Total: ${evals.length}`, ""];
  let n = 0;
  ...
  writeFileSync("/tmp/r23-nao-correto.md", md.join("\n"));
  console.log("Dump em: /tmp/r23-nao-correto.md");
