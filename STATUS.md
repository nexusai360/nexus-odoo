...
- **F6 Custo / Latência** (alvo 1-2 centavos/consulta) , **EM ANDAMENTO** (spec v1 + 2 reviews adversariais commitadas, `docs/superpowers/specs/2026-06-07-f6-custo-latencia-design.md`). **Descoberta-chave das reviews: grande parte da F6 JÁ EXISTE** , telemetria de custo (`LlmUsage`+`logUsage`+`calculateCost`+catalog.ts price table + UI `/agente/consumo`), `MAX_ITERATIONS=3`, slots de modelo por papel em `AgentSettings`. **F6 re-escopada (bloco CORRECOES [R] na spec):** Onda 1 = custo-por-CONSULTA agregando LlmUsage + harness de regressão de custo reusando o golden F5 + conta de referência; Onda 2 = ATIVAR o retrieval (hoje `routerToolRetrieval=shadow`, catálogo inteiro ainda vai ao LLM) sob gate recall@K+golden , o **maior ganho**; Onda 3 (com migration) = model-tiering em roteamento/classificação (NÃO redação , golden não protege texto; usar `claude-judge-runner`) + short-circuit 1-tool com guard de desambiguação + cache de roteamento RBAC-safe. Reconciliar com a spec `2026-06-03-otimizacao-custo-agente` (já entregou prompt caching/janela; descartou modelo-menor+reasoning). **PRÓXIMO: aplicar correções → spec v3 → plano (2 reviews) → execução TDD → E2E → PR → merge.**
