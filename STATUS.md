...
- **F6 Custo / Latência** (alvo 1-2 centavos/consulta) , **CÓDIGO COMPLETO, PR aberto** (spec v3 + plano v3 após 2+2 reviews adversariais; `docs/superpowers/specs/2026-06-07-f6-custo-latencia-design.md` + `docs/superpowers/plans/2026-06-08-f6-custo-latencia-plan.md`). **Conta de custo de referência (feita antes do plano): o alvo 1-2c JÁ é o patamar atual (~2c/consulta hoje); caching (entregue na spec 06-03) + retrieval active → ~0,96c. Model-tiering FORA (desnecessário + sem gate textual inline).** SEM migration.
  ...
  - **FORA do escopo (declarado na spec v3):** model-tiering, short-circuit 1-tool, cache de roteamento. Dependência: coordenar ordem de merge com `feat/router-ativacao-r2`. **PRÓXIMO: aval do usuário para merge do PR + (depois) rodar custo-fiel/Gate C no full-stack e promover a flag.**
