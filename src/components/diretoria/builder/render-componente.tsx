...
interface LinhaChave { chave: string; valorTotal: number }
interface DadoBarras { linhas: LinhaChave[] }
interface DadoVendasKpi { faturamento: number; numPedidos: number; ticketMedio: number; margemPct: number }
interface DadoModalidades extends DadoBarras { maiorPedido: { numero: string | null; participante: string | null; valor: number } | null }

function KpisVendas({ d }: { d: DadoVendasKpi }) {
  const cards = [
    { label: "Faturamento", valor: brl.format(d.faturamento) },
    { label: "Pedidos", valor: num.format(d.numPedidos) },
    { label: "Ticket médio", valor: brl.format(d.ticketMedio) },
    { label: "Margem estimada", valor: `${d.margemPct.toFixed(1)}%` },
  ];
  return (
    <div className="grid grid-cols-2 gap-3">
      {cards.map((c) => (
...
function BarrasChave({ linhas }: { linhas: LinhaChave[] }) {
  if (!linhas.length) return <p className="py-6 text-center text-sm text-muted-foreground">Sem dados.</p>;
...
function Modalidades({ d }: { d: DadoModalidades }) {
  return (
    <div className="flex flex-col gap-4">
      {d.maiorPedido ? (
        <div className="rounded-xl border border-violet-500/30 bg-violet-600/5 p-3">
          <div className="text-xs text-muted-foreground">Maior pedido</div>
          <div className="mt-1 flex items-baseline justify-between gap-2">
            <span className="truncate text-sm font-medium">
              {d.maiorPedido.numero ?? "-"} · {d.maiorPedido.participante ?? "-"}
            </span>
            <span className="shrink-0 text-sm font-semibold tabular-nums">{brl.format(d.maiorPedido.valor)}</span>
          </div>
        </div>
      ) : null}
      <BarrasChave linhas={d.linhas} />
    </div>
  );
}
...
export function renderComponente(id: string, dado: unknown): ReactNode {
  switch (id) {
    ...
    case "C-01":
      return <KpisVendas d={dado as DadoVendasKpi} />;
    case "C-02":
    case "C-03":
    case "C-07":
