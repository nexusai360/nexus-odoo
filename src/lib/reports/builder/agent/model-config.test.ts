...
describe("model-config do construtor", () => {
  ...
  it("retorna o default quando os campos estao vazios (null)", async () => {
    findUnique.mockResolvedValue({
      builderModelProvider: null,
      builderModelId: null,
      builderModelCredentialId: null,
    });
    const c = await obterConfigModeloConstrutor();
    expect(c).toEqual({
      provider: DEFAULT_BUILDER_PROVIDER,
  ...
  it("retorna o que esta gravado no singleton quando preenchido", async () => {
    findUnique.mockResolvedValue({
      ...
      builderModelCredentialId: "cred-123",
    });
    const c = await obterConfigModeloConstrutor();
    expect(c).toEqual({
      provider: "anthropic",
  ...
  it("grava provider+model+credencial no singleton via upsert (id global)", async () => {
    ...
    expect(arg.update).toMatchObject({
      ...
      builderModelCredentialId: "cred-9",
    });
