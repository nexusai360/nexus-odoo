...
function makePrisma() {
  return {
    ...
    fatoPreco: { findMany: jest.fn(), count: jest.fn() },
  };
}
...
describe("preco_produto , paginacao (alavanca 2b)", () => {
  it("aplica limit/offset no SQL (take/skip) e desempate estavel no orderBy", async () => {
    const ctx = makeCtx();
    primeFreshness(ctx);
    primeQuery(ctx, 100, 10);

    const r = await comercialPrecoProduto.handler({ termo: "Esteira", limit: 10, offset: 0 } as never, ctx);
  ...
  it("ultima pagina nao indica mais itens", async () => {
    ...
    const r = await comercialPrecoProduto.handler({ termo: "Esteira", limit: 10, offset: 10 } as never, ctx);
    const callArgs = (ctx.prisma.fatoPreco.findMany as jest.Mock).mock.calls[0][0];
  ...
  it("default limit = 50 quando ausente", async () => {
    ...
    await comercialPrecoProduto.handler({ termo: "Esteira" } as never, ctx);
    const callArgs = (ctx.prisma.fatoPreco.findMany as jest.Mock).mock.calls[0][0];
