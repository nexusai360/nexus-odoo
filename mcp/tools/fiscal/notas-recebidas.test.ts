...
function fakeLinhas(n: number) {
  return Array.from({ length: n }, (_, i) => ({
    ...
    participanteNome: `F${i + 1}`,
    vrNf: 10,
  }));
}

describe("fiscal_notas_recebidas , paginacao (alavanca 2b)", () => {
  it("aplica limit/offset (take/skip) e desempate estavel no orderBy", async () => {
    ...
    const r = await fiscalNotasRecebidas.handler({ limit: 10, offset: 0 } as never, ctx);
  ...
  it("default limit = 50 quando ausente", async () => {
    ...
    await fiscalNotasRecebidas.handler({} as never, ctx);
    const callArgs = (ctx.prisma.fatoNotaFiscal.findMany as jest.Mock).mock.calls[0][0];
