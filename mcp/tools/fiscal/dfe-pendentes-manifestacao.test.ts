...
describe("fiscal_dfe_pendentes_manifestacao , paginacao (alavanca 2b)", () => {
  it("aplica limit/offset no SQL (take/skip) e desempate estavel no orderBy", async () => {
    ...
    const r = await fiscalDfePendentesManifestacao.handler({ limit: 10, offset: 0 } as never, ctx);
  ...
  it("ultima pagina: temMais=false e proximoOffset=null", async () => {
    ...
    const r = await fiscalDfePendentesManifestacao.handler({ limit: 10, offset: 10 } as never, ctx);
    const callArgs = (ctx.prisma.fatoDfe.findMany as jest.Mock).mock.calls[0][0];
  ...
  it("default limit = 50 quando ausente", async () => {
    ...
    await fiscalDfePendentesManifestacao.handler({} as never, ctx);
    const callArgs = (ctx.prisma.fatoDfe.findMany as jest.Mock).mock.calls[0][0];
