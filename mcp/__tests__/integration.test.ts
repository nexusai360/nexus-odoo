...
// Harness de teste de integração do MCP — 4f-4 (Onda F: 33 tools; Bloco J: +1 = 34 tools).
//
// Cobre:
//   1. Assertiva de catálogo: super_admin recebe EXATAMENTE 34 tools com os IDs corretos (N6).
...
//   - catalogo: REAL (as 34 tools de produção — objetivo da rede N6)
...
describe("Catálogo completo — rede de proteção N6", () => {
  it("super_admin recebe EXATAMENTE 34 tools", () => {
  ...
  it("catálogo bruto (antes do filtro) tem exatamente 35 entradas", () => {
    // 34 tools de leitura + 1 write tool (crm.res_partner.create). A write tool
...
describe("Catálogo filtrado por perfil", () => {
  ...
  it("super_admin vê todas as 34 tools", () => {
  ...
  it("admin vê as 3 tools de cadastros (RBAC camada 1 — vê tudo)", () => {
    const ids = tools("admin", ["estoque", "financeiro"]);
...
describe("Servidor HTTP real — protocolo Streamable HTTP end-to-end", () => {
  ...
  it("super_admin: tools/list via HTTP retorna 34 tools com os IDs corretos", async () => {
