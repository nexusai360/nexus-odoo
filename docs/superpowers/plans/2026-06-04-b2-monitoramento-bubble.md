...
- [ ] **Step 1:** `queries.ts`: `fetchQualityEvaluationRowById(id)` , `getEvaluationDetail(id)` retorna superset (`evaluation: EvaluationRow & {...}`); fazer pick de TODOS os campos de `EvaluationRow` (`:43-61`), incluindo os obrigatórios `patterns: string[]` e `dominantPattern` (nomes reais; `getEvaluationDetail:450/457` já os traz), além de id, conversationId, status, humanStatus, model, questionSnapshot, answerSnapshot, rodada, channel, createdAt. Retornar `null` se não achar.
