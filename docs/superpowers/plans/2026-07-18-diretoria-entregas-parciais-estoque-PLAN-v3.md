...
- **D-b) Grão de "bloqueado"** `[R1:A7]` , **PENDENTE DE VEREDITO DO DONO (default SIMPLES adotado)**: por CLIENTE, **só título `a_receber` de NOTA FISCAL EMITIDA, vencido em aberto** (`vrSaldo>0`, `dataVencimento < inicioDoDia`, `dataDocumento >= corte`, `filtrarTitulosExternos`, **`notaFiscalId != null`** , carteira **NÃO** conta). Versão mais simples/branda, a pedido do dono (2026-07-18). **O dono vai verificar a definição correta na operação e passar o veredito depois , TA.2 fica isolada e fácil de re-parametrizar; LEMBRAR o dono disso no fecho.**
...
- **TA.2 , Bloqueio por cliente (query batched única).** `[R2:A-2]` `statusBloqueioPorCliente(prisma, participanteIds)`: **uma** query sobre `fato_financeiro_titulo` com o predicado de `queryTitulosVencidos` (`vrSaldo>0`, `dataVencimento < inicioDoDia(hoje)`, `dataDocumento >= corteAtualDate()`, `tipo="a_receber"`, `filtrarTitulosExternos`) **+ `notaFiscalId != null`** (versão simples de D-b: só nota emitida, carteira não conta), agrupando por `participanteId` em memória (sem N+1; tabela ~5,5k, full-scan ok). **A regra de bloqueio fica numa constante/flag isolada e comentada (`REGRA_BLOQUEIO`), para o dono trocar depois com um toque.** Testes: cliente com nota vencida / em dia / sem título / só carteira vencida (não bloqueia na versão simples).
