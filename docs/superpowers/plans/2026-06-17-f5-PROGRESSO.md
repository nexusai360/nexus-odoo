...
- [x] **Onda F.1 (runbook n8n)** , `docs/runbooks/2026-06-17-f5-whatsapp-n8n-runbook.md` (2 webhooks, HMAC, mapeamento Meta, `reason`s, idempotência/dedupe por deliveryId+inboundMessageId, breaking change do envelope, dívidas).
- [x] **Suíte completa verde:** jest 3136 passed / 27 skipped / **0 failed** (424 suites); tsc 0; eslint 0 erros (warnings pré-existentes). Achado e corrigido: teste pré-existente `__tests__/monitoramento-bubble.test.ts` fixava `channel:"in_app"` , migrado p/ `{in:[in_app,whatsapp]}`.
- [x] **MERGE para `main` + DEPLOY (autorizado pelo usuário 2026-06-17):** PR #127 squash-merged na main; build-app/build-mcp success; `prod /api/health: OK`. Deploy rolling forçado (`deploy-portainer.py`) para aplicar a migration F5 (aditiva) na prod imediatamente via `entrypoint.sh` (`migrate deploy`). Só 1 migration nova vs main, sem destrutivos. localhost:3000 no ar com a branch para validação visual.
- [ ] **PENDENTE:** (1) **DROP das colunas legadas** `bubbleEnabled`/`whatsappEnabled` , só DEPOIS que `feat/nex-reconstrucao` e `feat/deploy-producao` rebasearem na main (RADAR `R-f5-drop-booleans-legados`). (2) **Onda F.2 e2e contra dado real** , exige docker + payload Meta real + credenciais; cenários da spec §16 no plano. (3) Opcional: `/ultrareview`.
