...
- [ ] **PENDENTE (humano):** (1) **PR aberto** , aguarda decisão de merge para `main` (mergear CEDO: banco compartilhado; após merge, outras frentes rebaseiam e então roda-se o DROP deferido das colunas legadas , RADAR `R-f5-drop-booleans-legados`). (2) **Onda F.2 e2e contra dado real** , exige subir docker (app+mcp+worker) + payload Meta real + credenciais; cenários da spec §16 listados no plano. (3) Opcional: `/ultrareview`.
