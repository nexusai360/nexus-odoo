...
## CORRECOES [R] das 2 reviews adversariais (aplicar na v3 , F6 muito menor do que a v1 supos)

> As 2 reviews (workflow wvjb6yoau) acharam 5 criticos + altos: grande parte da F6 JA EXISTE. A v3 deve REUSAR, nao reconstruir.

1. **[CRITICO] Telemetria de custo JA EXISTE , nao recriar.** `LlmUsage` (prisma) grava por iteracao: costUsd, costBrl, costKnown, tokensInput/Output, tokensCachedInput, reasoningTokens, durationMs, toolCallsCount, conversationId, userId, origin. `run-agent.ts` ja chama `logUsage()` por iteracao; `calculateCost()` + tabela de precos versionada em `src/lib/agent/llm/catalog.ts`; `usage-stats.ts` ja agrega; UI `/agente/consumo` ja consome. **O delta REAL da F6:** (a) custo por **CONSULTA** (= soma de LlmUsage por conversationId/turno; hoje so por iteracao) , inventariar TODAS as chamadas LLM do turno (enhance, guardrail, autoValidator retry) e garantir `logUsage` com `origin` distinto; (b) o gate de regressao consumir esse agregado. **`estimarCustoUsd` = wrapper fino sobre `calculateCost`, nao reimplementar.**
2. **[CRITICO] Reconciliar com a SPEC `2026-06-03-otimizacao-custo-agente-design.md`** (v3, revisada 2x): ela JA entregou prompt caching, janela de historico, paginacao, e **DESCARTOU modelo-menor-na-1a-chamada e reasoning-effort**. A v3 da F6 deve listar o que aquela entregou e decidir explicitamente (com aval do usuario) se reabre algo , nao reabrir decisao canonica em silencio.
3. **[CRITICO] Golden F5 NAO protege a REDACAO** (mede numero/selecao/alucinacao/desambiguacao, nao qualidade textual). Logo: model-tiering na `redacao_final` ou (a) usa o `claude-judge-runner` (judge de redacao ja existente) como gate obrigatorio, ou (b) fica **FORA DE ESCOPO** , tier so em roteamento/classificacao (saida estruturada, onde o golden cobre).
4. **[CRITICO] Sem migration para telemetria** (reusa LlmUsage). Model-tiering reusa o padrao de slots por papel ja em `AgentSettings` (intelligenceModel/qualityJudgeModel/routerReformModel/Provider/CredentialId; resolver `resolveReformLlm` em get-reform-config.ts) , adicionar slot de papel e migration (manual + migrate deploy, checar drift). NAO mexer em `getActiveLlmConfig`. Ondular: telemetria (sem schema) primeiro; tiering (com migration) depois.
5. **[ALTO] Retrieval esta em SHADOW** (`routerToolRetrieval` default 'shadow', `routerEnabled` default false): o catalogo INTEIRO ainda vai ao LLM; o retrieval calcula mas NAO corta tokens. O lever "retrieval ja barateia" e na verdade **ativar sob gate** (promover para active/true) com **recall@K>=98% + golden F5 verde** como condicao. Esse e provavelmente o MAIOR ganho de custo da F6.
6. **[ALTO] Caches existentes:** `session-cache.ts` cacheia RESULTADO de tool (numero!) por conversationId, TTL 60s , **contradiz** a decisao "nunca cachear numero" da v1; decidir manter (ajustar a decisao) ou reduzir. `embed-question.ts` ja tem LRU de 200 embeddings. Cache de ROTEAMENTO = user-agnostico do dado, TTL puro, **chave com a assinatura de permissao RBAC** (nao so user.domains , RBAC v2 e mais fino). Cache de ENTIDADE depende do dado , precisa invalidacao por versao de sync (NAO existe hoje; e trabalho da F6 se entrar).
7. **[ALTO] Step limit JA EXISTE** (`MAX_ITERATIONS=3` em run-agent.ts). O novo e o **short-circuit 1-tool**, condicionado a `routerToolRetrieval='active'` + `classifyIntent==='pontual'` + top-1 acima de limiar + **todos os args obrigatorios resolvidos sem ambiguidade** (reusar F2; rodar o subconjunto desambiguacao do golden para provar que nenhum virou chute).
8. **[ALTO] Conta de custo de referencia ANTES do plano:** decompor o custo atual por papel/passo (via calculateCost) e mostrar qual lever reduz quanto; se sem mexer no input da redacao (caching/modelo/reasoning) nao se chega a 1-2c, declarar e renegociar a meta/escopo com o usuario , nao depois.
9. **[MEDIO] costKnown:** o harness de custo deve marcar 'indisponivel'/falhar quando uma fracao das requisicoes vier `costKnown=false`/tokens=0, nunca somar 0 em silencio.
10. **[MEDIO] Cenario fixo no snapshot de custo:** registrar modelo/flags/routerToolRetrieval/autoValidatorMode no snapshot; comparar custo so entre execucoes do MESMO cenario (mudar flag => nova baseline).

**Resumo do re-escopo:** F6 vira: (Onda 1) custo-por-consulta agregando LlmUsage + harness de regressao de custo reusando o golden (qualidade+custo juntos) + conta de referencia. (Onda 2) ATIVAR retrieval sob gate (recall@K+golden) , o maior ganho. (Onda 3, opcional/com migration) model-tiering em roteamento/classificacao (nao redacao, ou com judge) + short-circuit 1-tool com guard de desambiguacao + cache de roteamento RBAC-safe. Telemetria, step-limit e tabela de precos NAO se reconstroem.
